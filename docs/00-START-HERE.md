...
| Plans v2 | Pro → **استارتر (Starter)**, Featured → **پریمیوم (Premium)** (display names only — `PlanId` stays `pro`/`featured`, see `lib/billing/plans.ts`). Two tiered features shipped: **gallery** (free 3 photos, Starter 5+1 video, Premium unlimited) and **review replies** (Starter+, owner writes a public reply, gated server-side). Announcements, vanity URLs, and the rest of the brainstormed Premium list are open-tasks backlog, not built — Notion Mission Control write-up is pending a connector outage (503) |
