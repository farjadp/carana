...
**Written:** 2026-08-24 · **Updated:** 2026-08-18 · **Docs version:** 3.4
...
| Mobile | **APK 1.2.0 built and linked 16 Aug** (`229669c`) — it fixed a real outage: 1.1.0 shipped with no Supabase credentials and could not start at all. **1.2.0 has still never run on a real Android device.** Mobile now has the *read* side of everything (announcements ×3 surfaces, busy status, FX/clock card, features screen, **the jobs board**) but **no owner controls at all** — no edit, insights, billing, or announcement writing |
