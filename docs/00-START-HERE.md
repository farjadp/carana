...
| Plans v2 | Pro → **استارتر (Starter)**, Featured → **پریمیوم (Premium)** (display names only — `PlanId` stays `pro`/`featured`, see `lib/billing/plans.ts`). Four tiered features shipped: **gallery**, **review replies**, **busy now/quiet now** (display now on mobile too — expiry logic moved to `@charana/core`), **announcements**. Vanity URLs and 6 more Premium items remain, plus mobile has no owner-management screens at all yet (registration only) — tracked in Notion Mission Control |
