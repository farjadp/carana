...
| Plans v2 | Pro → **استارتر (Starter)**, Featured → **پریمیوم (Premium)** (display names only — `PlanId` stays `pro`/`featured`, see `lib/billing/plans.ts`). Three tiered features shipped: **gallery** (free 3/Starter 5+1 video/Premium unlimited), **review replies** (Starter+, public, gated server-side), **busy now/quiet now** (Starter+, self-expiring 4h toggle, shown on every `BusinessCard` and the profile hero). Announcements, vanity URLs, and the rest of the Premium backlog are tracked as 11 Ready items in Notion Mission Control |
