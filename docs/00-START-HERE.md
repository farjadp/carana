...
**Written:** 2026-08-24 · **Updated:** 2026-08-26 (channels directory **live** — «کانال‌ها و گروه‌ها»; it replaced «چرا گوپلازا؟» on the home page; header rebuilt) · **Docs version:** 3.19
...
| Blog | 7 categories, fal.ai brand imagery; live on web **and in the app**. **Two writers:** the data-driven one runs 11:00 UTC, the **source-driven** one (atash.ca → our own article, cited) runs 12:00 UTC — both into the review queue. Posts carry a `key_takeaway` answer block for AIO/GEO. Telegram (`@GoPlaza`) + LinkedIn syndication is built and env-gated; a paced backlog drip runs 15:00 UTC once the credentials are set. See `13-blog-sources.md`. **Its migration `20260830320000_blog_sources.sql` still has to be run in the SQL Editor.** **Discovery (24 Aug):** it is no longer one link in a dropdown — an editorial band on the home page (lead post + three rows + category chips) and a three-card «جدیدترین مقالات» strip above the footer on the business, city, province, category and job pages, both from `components/blog/latest-posts.tsx`, both rendering nothing when no post is published. «مقالات» sits in the header bar; the desktop nav breakpoint moved 900→960px to make room, see `06-gotchas` |
