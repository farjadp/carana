// ============================================================================
// Source: components/ui/rail.tsx
// Version: 1.0.0 — 2026-08-27 (extracted from blog/posts-rail.tsx 1.1.0)
// Why: A horizontal rail of cards, 3–4 in view at a time. Touch and trackpads
//      scroll it natively; this client shell only adds the two arrow buttons a
//      mouse needs, and hides each arrow when that end is reached. The cards
//      are passed in as children, so they stay server-rendered.
//
//      Extracted unchanged from the blog rail when the home page's «جدیدترین»
//      and «پربازدیدترین» sections became rails too. The RTL scrolling below
//      cost a day to get right; a second copy of it would have drifted.
//
...
export function Rail({
  ...
  /** Aria label for the arrow that scrolls back towards the first card. */
  prevLabel: string;
  /** Aria label for the arrow that scrolls on towards the last card. */
  nextLabel: string;
}) {
  ...
  return (
    <div className="relative">
      ...
      {/* Arrows: desktop only — a touch rail scrolls itself. The rail is RTL,
          so its start is the RIGHT edge; the right arrow goes back towards the
          first card and the left arrow onwards to the last. */}
      {!atStart ? (
        <RailArrow side="right" onClick={() => nudge(1)} label={prevLabel}>
          <ChevronRight className="h-5 w-5" />
        </RailArrow>
      ) : null}
      {!atEnd ? (
        <RailArrow side="left" onClick={() => nudge(-1)} label={nextLabel}>
